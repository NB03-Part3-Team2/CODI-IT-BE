...
describe('UserService 단위 테스트', () => {
  ...
  describe('createUser 메소드 테스트', () => {
    test('createUser 성공 테스트 - mock방식', async () => {
      ...
      const originalPassword = userCreateDto.password; // 원본 비밀번호 저장

      const mockCreatedUser: UserCreatedDto = {
        id: 'user123',
        gradeId: 'grade123',
        name: '테스트유저',
        email: 'test@example.com',
      ...
      expect(getUserByEmailMock).toHaveBeenCalledWith(userCreateDto.email);
      expect(getUserByNameMock).toHaveBeenCalledWith(userCreateDto.name);
    ...
    test('createUser 실패 테스트 - 이메일 중복', async () => {
      ...
      // 에러가 발생하는지 확인합니다.
      await expect(userService.createUser(userCreateDto)).rejects.toMatchObject({
        code: 409,
